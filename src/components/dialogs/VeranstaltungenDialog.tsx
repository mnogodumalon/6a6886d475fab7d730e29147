...
export type VeranstaltungenDialogDefaults = Omit<Veranstaltungen['fields'], 'veranstaltung_status'> & {
    veranstaltung_status?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  veranstaltung_status: LOOKUP_OPTIONS['veranstaltungen']?.['veranstaltung_status'] ?? [],
};
...
export function VeranstaltungenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['veranstaltungsname', 'start_datum', 'veranstaltung_status'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "veranstaltungsname": string | null, // Veranstaltungsname\n  "start_datum": string | null, // YYYY-MM-DDTHH:MM\n  "ende_datum": string | null, // YYYY-MM-DDTHH:MM\n  "beschreibung": string | null, // Beschreibung\n  "verantwortliche_person": string | null, // Verantwortliche Person\n  "max_teilnehmer": number | null, // Maximale Teilnehmerzahl\n  "angemeldete_teilnehmer": number | null, // Aktuell angemeldete Teilnehmer\n  "veranstaltung_status": LookupValue | null, // Status (select one key: "geplant" | "bestaetigt" | "abgesagt") mapping: geplant=Geplant, bestaetigt=Bestätigt, abgesagt=Abgesagt\n  "bemerkungen_veranstaltung": string | null, // Bemerkungen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'veranstaltungsname': (
      <div key="veranstaltungsname" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.veranstaltungsname && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'start_datum': (
      <div key="start_datum" className="space-y-1.5">
        ...
        <DatePicker
          ...
          required
        />
        {showErrors && !fields.start_datum && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'ende_datum': (
      <div key="ende_datum" className="space-y-1.5">
    ...
    'angemeldete_teilnehmer': (
      <div key="angemeldete_teilnehmer" className="space-y-1.5">
        <Label htmlFor="angemeldete_teilnehmer">Aktuell angemeldete Teilnehmer</Label>
        <Input
    ...
    'veranstaltung_status': (
      <div key="veranstaltung_status" className="space-y-1.5">
        <Label htmlFor="veranstaltung_status">Status <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.veranstaltung_status) === 'geplant'}
            onClick={() => setFields(f => ({ ...f, veranstaltung_status: (lookupKey(f.veranstaltung_status) === 'geplant' ? undefined : 'geplant') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.veranstaltung_status) === 'geplant'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.veranstaltung_status) === 'bestaetigt'}
            onClick={() => setFields(f => ({ ...f, veranstaltung_status: (lookupKey(f.veranstaltung_status) === 'bestaetigt' ? undefined : 'bestaetigt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.veranstaltung_status) === 'bestaetigt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.veranstaltung_status) === 'abgesagt'}
            onClick={() => setFields(f => ({ ...f, veranstaltung_status: (lookupKey(f.veranstaltung_status) === 'abgesagt' ? undefined : 'abgesagt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.veranstaltung_status) === 'abgesagt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
        ...
        {showErrors && !fields.veranstaltung_status && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"veranstaltungsname": "Veranstaltungsname", "start_datum": "Beginn (Datum & Uhrzeit)", "ende_datum": "Ende (Datum & Uhrzeit)", "beschreibung": "Beschreibung", "verantwortliche_person": "Verantwortliche Person", "max_teilnehmer": "Maximale Teilnehmerzahl", "angemeldete_teilnehmer": "Aktuell angemeldete Teilnehmer", "veranstaltung_status": "Status", "bemerkungen_veranstaltung": "Bemerkungen"};

/**
 * ReservierungenDialog — pre-generated create/edit dialog for Reservierungen.
 *
 ...
 * ReservierungenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Reservierungen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<ReservierungenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Reservierungen, Tische, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Reservierungen';
...
/** Widened prefill type for ReservierungenDialog.defaultValues — see file header. */
export type ReservierungenDialogDefaults = Omit<Reservierungen['fields'], 'bestaetigung'> & {
    bestaetigung?: LookupValue | string;
  };

interface ReservierungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Reservierungen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: ReservierungenDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  bestaetigung: LOOKUP_OPTIONS['reservierungen']?.['bestaetigung'] ?? [],
};
...
export function ReservierungenDialog({ open, onClose, onSubmit, defaultValues, recordId, tischeList, enablePhotoScan = true, enablePhotoLocation = true }: ReservierungenDialogProps) {
  const [fields, setFields] = useState<Partial<Reservierungen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['datum_uhrzeit', 'personenanzahl', 'tisch', 'gast_vorname', 'gast_nachname', 'gast_telefon', 'bestaetigung'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Reservierungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'reservierungen');
      await onSubmit(clean as Reservierungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "datum_uhrzeit": string | null, // YYYY-MM-DDTHH:MM\n  "personenanzahl": number | null, // Anzahl Personen\n  "tisch": string | null, // Display name from Tische (see <available-records>)\n  "gast_vorname": string | null, // Vorname\n  "gast_nachname": string | null, // Nachname\n  "gast_telefon": string | null, // Telefonnummer\n  "gast_email": string | null, // E-Mail-Adresse\n  "anlass": string | null, // Anlass\n  "bestaetigung": LookupValue | null, // Bestätigungsstatus (select one key: "bestaetigt" | "nicht_bestaetigt" | "storniert") mapping: bestaetigt=Bestätigt, nicht_bestaetigt=Nicht bestätigt, storniert=Storniert\n  "bemerkungen_reservierung": string | null, // Interne Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Reservierungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Reservierungen bearbeiten' : 'Reservierungen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'personenanzahl': (
      <div key="personenanzahl" className="space-y-1.5">
        <Label htmlFor="personenanzahl">Anzahl Personen <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="personenanzahl"
          type="number"
          step="any"
        ...
        />
        {showErrors && !fields.personenanzahl && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'tisch': (
      <div key="tisch" className="space-y-1.5">
        ...
        <Combobox
          ...
          createLabel="Neu in Tische"
        />
        {showErrors && !fields.tisch && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    ...
    'gast_telefon': (
      <div key="gast_telefon" className="space-y-1.5">
        <Label htmlFor="gast_telefon">Telefonnummer <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="gast_telefon"
          value={fields.gast_telefon ?? ''}
          onChange={e => setFields(f => ({ ...f, gast_telefon: e.target.value }))}
        />
        {showErrors && !fields.gast_telefon && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    ...
    'bestaetigung': (
      <div key="bestaetigung" className="space-y-1.5">
        <Label htmlFor="bestaetigung">Bestätigungsstatus <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bestaetigung) === 'bestaetigt'}
            onClick={() => setFields(f => ({ ...f, bestaetigung: (lookupKey(f.bestaetigung) === 'bestaetigt' ? undefined : 'bestaetigt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bestaetigung) === 'bestaetigt'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bestaetigung) === 'nicht_bestaetigt'}
            onClick={() => setFields(f => ({ ...f, bestaetigung: (lookupKey(f.bestaetigung) === 'nicht_bestaetigt' ? undefined : 'nicht_bestaetigt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bestaetigung) === 'nicht_bestaetigt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bestaetigung) === 'storniert'}
            onClick={() => setFields(f => ({ ...f, bestaetigung: (lookupKey(f.bestaetigung) === 'storniert' ? undefined : 'storniert') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bestaetigung) === 'storniert'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
        ...
        {showErrors && !fields.bestaetigung && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'bemerkungen_reservierung': (
      <div key="bemerkungen_reservierung" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"datum_uhrzeit": "Datum & Uhrzeit", "personenanzahl": "Anzahl Personen", "tisch": "Tisch", "gast_vorname": "Vorname", "gast_nachname": "Nachname", "gast_telefon": "Telefonnummer", "gast_email": "E-Mail-Adresse", "anlass": "Anlass", "bestaetigung": "Bestätigungsstatus", "bemerkungen_reservierung": "Interne Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"tisch": {"tischnummer": "Tischnummer", "bereich": "Bereich", "kapazitaet": "Kapazität (Sitzplätze)", "status": "Status", "bemerkungen_tisch": "Bemerkungen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.RESERVIERUNGEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createTischeOpen && (
      <TischeDialog
        ...
        defaultValues={createTischeInitial
          ? ({ tischnummer: createTischeInitial } as any)
          : undefined}
      />
    )}
    </>

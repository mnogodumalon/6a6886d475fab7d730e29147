...
import type { Tische, Reservierungen, Veranstaltungen, Speisekarte, CreateTische, CreateReservierungen, CreateVeranstaltungen, CreateSpeisekarte } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- RESERVIERUNGEN ---
  static async getReservierungen(): Promise<Reservierungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.RESERVIERUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Reservierungen[];
    return enrichLookupFields(records, 'reservierungen');
  }
  static async getReservierungenEntry(id: string): Promise<Reservierungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.RESERVIERUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Reservierungen;
    return enrichLookupFields([record], 'reservierungen')[0];
  }
  static async createReservierungenEntry(fields: CreateReservierungen) {
    return callApi('POST', `/apps/${APP_IDS.RESERVIERUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'reservierungen') });
  }
  static async updateReservierungenEntry(id: string, fields: Partial<CreateReservierungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.RESERVIERUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'reservierungen') });
  }
  static async deleteReservierungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.RESERVIERUNGEN}/records/${id}`);
  }

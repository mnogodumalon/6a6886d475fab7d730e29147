...
import { ReservierungenViewDialog } from '@/components/dialogs/ReservierungenViewDialog';
...
const TISCHE_FIELDS = [
  { key: 'tischnummer', label: 'Tischnummer', type: 'string/text' },
  { key: 'bereich', label: 'Bereich', type: 'lookup/select', options: [{ key: 'innenraum', label: 'Innenraum' }, { key: 'terrasse', label: 'Terrasse' }, { key: 'bar', label: 'Bar' }, { key: 'privatraum', label: 'Privatraum' }, { key: 'garten', label: 'Garten' }] },
  { key: 'kapazitaet', label: 'Kapazität (Sitzplätze)', type: 'number' },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'verfuegbar', label: 'Verfügbar' }, { key: 'reserviert', label: 'Reserviert' }, { key: 'gesperrt', label: 'Gesperrt' }] },
  { key: 'bemerkungen_tisch', label: 'Bemerkungen', type: 'string/textarea' },
];
const RESERVIERUNGEN_FIELDS = [
  { key: 'datum_uhrzeit', label: 'Datum & Uhrzeit', type: 'date/datetimeminute' },
  { key: 'personenanzahl', label: 'Anzahl Personen', type: 'number' },
  ...
  { key: 'anlass', label: 'Anlass', type: 'string/text' },
  { key: 'bestaetigung', label: 'Bestätigungsstatus', type: 'lookup/radio', options: [{ key: 'bestaetigt', label: 'Bestätigt' }, { key: 'nicht_bestaetigt', label: 'Nicht bestätigt' }, { key: 'storniert', label: 'Storniert' }] },
  { key: 'bemerkungen_reservierung', label: 'Interne Bemerkungen', type: 'string/textarea' },
];
const VERANSTALTUNGEN_FIELDS = [
  { key: 'veranstaltungsname', label: 'Veranstaltungsname', type: 'string/text' },
  { key: 'start_datum', label: 'Beginn (Datum & Uhrzeit)', type: 'date/datetimeminute' },
  { key: 'ende_datum', label: 'Ende (Datum & Uhrzeit)', type: 'date/datetimeminute' },
  ...
  { key: 'angemeldete_teilnehmer', label: 'Aktuell angemeldete Teilnehmer', type: 'number' },
  { key: 'veranstaltung_status', label: 'Status', type: 'lookup/radio', options: [{ key: 'geplant', label: 'Geplant' }, { key: 'bestaetigt', label: 'Bestätigt' }, { key: 'abgesagt', label: 'Abgesagt' }] },
  { key: 'bemerkungen_veranstaltung', label: 'Bemerkungen', type: 'string/textarea' },
];
const SPEISEKARTE_FIELDS = [
  { key: 'gerichtname', label: 'Gerichtname', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'vorspeise', label: 'Vorspeise' }, { key: 'suppe', label: 'Suppe' }, { key: 'hauptgang', label: 'Hauptgang' }, { key: 'beilage', label: 'Beilage' }, { key: 'dessert', label: 'Dessert' }, { key: 'getraenk', label: 'Getränk' }, { key: 'tagesgericht', label: 'Tagesgericht' }, { key: 'kinderteller', label: 'Kinderteller' }] },
  { key: 'beschreibung_gericht', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'preis', label: 'Preis (€)', type: 'number' },
  { key: 'allergene', label: 'Allergene & Hinweise', type: 'string/textarea' },
  { key: 'verfuegbarkeit', label: 'Verfügbarkeit', type: 'lookup/radio', options: [{ key: 'verfuegbar', label: 'Verfügbar' }, { key: 'nicht_verfuegbar', label: 'Nicht verfügbar' }, { key: 'saisonal', label: 'Saisonal' }] },
  { key: 'bild_gericht', label: 'Bild des Gerichts', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'tische', label: 'Tische', pascal: 'Tische' },
  { key: 'reservierungen', label: 'Reservierungen', pascal: 'Reservierungen' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('tische');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'tische': new Set(),
    'reservierungen': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'reservierungen': return (data as any).reservierungen as Reservierungen[] ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'reservierungen' && fieldKey === 'tisch') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'reservierungen': return RESERVIERUNGEN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'reservierungen': return {
        create: (fields: any) => LivingAppsService.createReservierungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateReservierungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteReservierungenEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'reservierungen' || dialogState?.entity === 'reservierungen') && (
        <ReservierungenDialog
          open={createEntity === 'reservierungen' || dialogState?.entity === 'reservierungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'reservierungen' ? handleUpdate : (fields: any) => handleCreate('reservierungen', fields)}
          defaultValues={dialogState?.entity === 'reservierungen' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'reservierungen' && (
        <ReservierungenViewDialog
          open={viewState?.entity === 'reservierungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'reservierungen', record: r }); }}
          tischeList={(data as any).tische ?? []}
        />
      )}

...
import { formEnhancements } from '@/config/form-enhancements/Tische';
import { evalComputed } from '@/config/form-enhancements/types';

export default function TischeDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Tische | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getTische();
  ...
  async function handleUpdate(fields: Tische['fields']) {
    if (!record) return;
    await LivingAppsService.updateTischeEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteTischeEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/tische');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/tische')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/tische')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.tischnummer ?? 'Tische'} />
